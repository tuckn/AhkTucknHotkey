...
IME が ON のときだけ、キャレット（文字の入力位置）の右下に赤丸を表示できます。日本語入力を日の丸に重ねた目印です。既定は直径6pxで、スペルチェックの下線と形で区別できます。
...
- `color`: `#RRGGBB` または `RRGGBB`。未設定・不正値は赤 `#E04040`。
- `shape`: `ellipse`（楕円）または `rectangle`（長方形）。未設定・不正値は `ellipse`。幅と高さが等しければ円になります。
- `width_px` / `height_px`: 幅・高さ。各2〜32px、既定は各 `6`。
- `offset_x_px` / `offset_y_px`: キャレット左下を基準にした、目印左上の相対位置。各-64〜64px、既定は `3` / `-3`。正値で右／下、負値で左／上に移動します。

寸法・位置は画面上のピクセル単位です。数値が未設定・不正・範囲外の場合は、その項目だけ既定値を使います。
短い縦線は `shape=rectangle`、`width_px=3`、`height_px=10`、`offset_y_px=-15`、横線は `width_px=10`、`height_px=2`、`offset_y_px=-4` で表現できます。
既存の `size_px` があり、`shape`・`width_px`・`height_px` がすべて未設定なら、旧来の横線（幅3〜24px、既定10px・高さ2px・位置5/-4）を維持します。赤丸へ切り替える場合は上の設定を適用してください。新しい形・寸法設定があれば `size_px` は使いません。
